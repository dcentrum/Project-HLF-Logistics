...
module.exports = (function() {
  return {
    get_all_shipments: function(req, res) {
      console.log("GETTING ALL SHIPMENTS FROM BLOCKCHAIN.... ");
      ...
      Fabric_Client.newDefaultKeyValueStore({
        ...
        // queryAllvehicle - requires no arguments , ex: args: [''],
        const request = {
          chaincodeId: 'BLOCK',
          txId: tx_id,
          fcn: 'getAllShipments',
    ...
    add_shipment: function(req, res) {
      ...
      Fabric_Client.newDefaultKeyValueStore({
        ...
        // recordvehicle - requires 5 args, ID, vessel, location, timestamp,holder - ex: args: ['10', 'Hound', '-12.021, 28.012', '1504054225', 'Hansel'],
    ...
    query_shipment: function(req, res) {
      ...
      Fabric_Client.newDefaultKeyValueStore({
        ...
        // queryvehicle - requires 1 argument, ex: args: ['4'],
        const request = {
          chaincodeId: 'BLOCK',
          txId: tx_id,
          fcn: 'getShipmentDetails',
          args: [bookingNo]
        };
        ...
        if (query_responses && query_responses.length == 1) {
          if (query_responses[0] instanceof Error) {
            console.error("error from query = ", query_responses[0]);
            res.send("Could not locate vehicle")
    ...
    update_package: function(req, res) {
      console.log("CHANGING THE PACKAGE STATUS OF A SHIPMENT.... ");
      ...
      Fabric_Client.newDefaultKeyValueStore({
        ...
        // changevehicleHolder - requires 2 args , ex: args: ['1', 'Barry'],
        // send proposal to endorser
        var request = {
          //targets : --- letting this default to the peers assigned to the channel
          chaincodeId: 'BLOCK',
          fcn: 'updatePackage',
          args: [RWBNo, bookingNo],
          chainId: 'blockchannel',
          txId: tx_id
        };
        ...
        if (isProposalGood) {
          ...
          console.error('Failed to send Proposal or receive valid response. Response null or status is not 200. exiting...');
          res.send("Error: no vehicle catch found");
          // throw new Error('Failed to send Proposal or receive valid response. Response null or status is not 200. exiting...');
        }
      }).then((results) => {
        console.log('Send transaction promise and event listener promise have completed');
